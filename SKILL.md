...
- **分析报告.md**：按 `assets/分析报告_report.md` 模板输出；浪型图以 Markdown 图片语法嵌入，**必须使用绝对路径**（如 `![主方案全景图](C:/绝对路径/主方案_全景.png)`），保证报告中图片可渲染；
- **网页报告.html**：复制 `assets/网页报告_web_report.html` **直接填充**生成（所有 `{{字段名}}` 替换为本次分析数据，图区填入 PNG 相对路径，如 `图表/主方案_全景.png`），禁止从零重写 HTML 结构；
- 两份输出中主备方案、失效价位、概率排序、简洁总结、关键价位表必须一致。

**不再交付**：CSV 数据文件（原始行情只用于分析过程）、独立 PNG 文件（PNG 仅用于嵌入 .md 与 .html）。
...
**输出归档（必须遵守）**：所有分析输出统一归档到项目根目录 `分析输出/`（在本仓库内即 `C:\games\Softwares\Codex\波浪skill\分析输出\`）。**一次分析 = 一个独立子文件夹**，命名 `标的名称_代码_YYYYMMDD_HHMM`（本地时间 4 位；Windows 文件夹名禁止冒号，如 `科创50_1B0688_20260809_1522`）；分析报告.md、网页报告.html、浪型图 PNG（可放 `图表/` 子目录）、原始行情 CSV、失效追踪、交易计划、变更日志全部放入该文件夹。同一标的的后续更新写回原文件夹；确需从头重开时新建带新时间戳的文件夹并在旧文件夹变更日志中登记去向。
...
2. **浪型标注图（放在总结之前）**：每个方案两张图（全景 + 放大，图片数量 = 候选方案数 × 2，见 references/rules.md R24）；每张图只用 Python（matplotlib）绘制 K 线再叠加浪型标注，保存 PNG 后以绝对路径嵌入 .md、以相对路径填入 .html（脚本 `scripts/ew_chart.py`；自检见 `references/workflow.md` 第 4 步第 2.1 节）；标题含级别与时间范围（见 references/rules.md R22）；浪段标注不混用（见 references/rules.md R23）；每段浪注明形态类型；失效价位红色虚线 + 规则编号（见 references/rules.md R19、R20）；标出支撑/阻力与当前价格。
